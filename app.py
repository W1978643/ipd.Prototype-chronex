...
class User(UserMixin, database.Model):
    ...
    display_name = database.Column(database.String(100), nullable=False)
...
class Goal(database.Model):
    """Savings goal model - budget target for user."""
    id = database.Column(database.Integer, primary_key=True)
    user_id = database.Column(database.Integer, database.ForeignKey('user.id'), nullable=False)
    goal_name = database.Column(database.String(100), nullable=False)
    target_amount = database.Column(database.Float, nullable=False)
...
def handle_registration():
    ...
    new_user = User(
        display_name=username.capitalize(),  # Capitalize for display
        username=username,
...
def show_budget_page():
    ...
    # Get user's savings goal (if set)
    savings_goal = Goal.query.filter_by(user_id=current_user.id).first()
    
    # Calculate estimated months to reach goal
    estimated_months_to_goal = None
    current_balance = total_income - total_expenses
    
    if savings_goal and current_balance > 0:
        # Estimate based on average monthly savings rate
        monthly_savings_rate = (total_income - total_expenses) / 12
        remaining_amount = savings_goal.target_amount - current_balance
        
        if current_balance >= savings_goal.target_amount:
